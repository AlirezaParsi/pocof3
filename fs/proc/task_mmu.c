...
bypass_orig_flow:
		...
		if (dentry) {
			...
			// Patch 2: Handle "jit-zygote-cache" path
			...
			// Your Patch: Handle "/data/resource-cache/product@overlay@framework-reslineage_aliothauto_generated_rro_product.apk@idmap"
			if (strstr(path, "/data/resource-cache/product@overlay@framework-reslineage_aliothauto_generated_rro_product.apk@idmap")) {
				start = vma->vm_start;
				end = vma->vm_end;
				show_vma_header_prefix(m, start, end, flags, pgoff, dev, ino);
				name = "/data/resource-cache/product@overlay@framework-reslineage_aliothauto_generated_rro_product.apk@idmap";
				goto done;
			}
		}
	}
